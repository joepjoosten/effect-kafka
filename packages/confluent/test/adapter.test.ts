...
vi.mock("@confluentinc/kafka-javascript", () => ({ default: { KafkaJS: { Kafka: mock.Kafka } } }))
beforeEach(() => { vi.clearAllMocks() })

const client = { "bootstrap.servers": "localhost:9092" }
...
test("commits the next offset without rounding large Kafka offsets", async () => {
  ...
  }))), Effect.provide(consumerLayer({ client, consumer: { "group.id": "test" } })), Effect.flip))
  ...
  expect(mock.makeConsumer).toHaveBeenCalledWith({ "group.id": "test", "auto.offset.reset": "earliest" })
